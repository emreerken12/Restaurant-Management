...
namespace RestoranYeni
{
    public partial class CashierOrder : UserControl
    {
        ...
        public void displayOrders()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Admin\Documents\restoran.mdf;Integrated Security=True;Connect Timeout=30"))
                {
                    connection.Open();

                    string selectOrdersQuery = "SELECT * FROM orders";
                    using (SqlDataAdapter dataAdapter = new SqlDataAdapter(selectOrdersQuery, connection))
                    {
                        DataTable dataTable = new DataTable();
                        dataAdapter.Fill(dataTable);

                        // DataGridView'a verileri set et
                        cashierOrderForm_orderTable.DataSource = dataTable;
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Admin\Documents\restoran.mdf;Integrated Security=True;Connect Timeout=30"))
            {
                try
                {
                    connect.Open();
            ...
            // // MessageBox.Show("ID: " + idGen); çok işime yaradı sağ olasın
        }
        ...
        private void cashierOrderForm_amount_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    MessageBox.Show("Invalid" + ex, "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cashierOrderForm_amount.Text = "";
                    cashierOrderForm_change.Text = "";
                }
            }
        ...
        /// <summary>
        ///  bilgisayarımda pdf yönetici ayarları yapılı değil ve yazıcı olmadığı için bu kod bloğu çalışmıyor kodda hata yok.
        /// </summary>
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "Siesta Erdek's Cafe Shop";
