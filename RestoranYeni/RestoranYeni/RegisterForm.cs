...
namespace RestoranYeni
{
    public partial class RegisterForm : Form
    {
        ...
        private void register_btn_Click(object sender, EventArgs e)
        {
            if (emptyFields())
            {
                MessageBox.Show("All fields are required to be filled.", "Eror Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand checkUserName = new SqlCommand(selectUsername, connect))
                        {
                            ...
                            {
                                MessageBox.Show("Password max 8 character", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            else
                            {
                                string insertDate = "INSERT INTO users(username, password, profile_image , role, status, date_reg)" +
